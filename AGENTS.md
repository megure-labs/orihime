...
Orihime is the public repository for the `py-d2p` distribution and `d2p`
Python package. Keep those package and import names stable unless a maintainer
explicitly requests an API migration.
...
the public CPU/CUDA and PyTorch compatibility contracts, and update tests and
...
```bash
python -m compileall -q d2p tests
python -m pytest -q
```
