...
``vjp_one``, ``vjp``; see :mod:`orihime.raw._base`) alongside the named kernel
...
reach these bindings. Prefer the top-level functions or ``orihime.nn`` for normal
model code.
"""
...
from . import sv_affine as sv_affine
from . import sv_linear as sv_linear
