...
def _validate_numerical_domain(
    ...
    *,
    ...
    message = (
        "orihime's supported FP32 numerical domain requires every finite "
...
def _cast_compute_dtype(
    ...
    if dtype != torch.float32:
        raise ValueError(
            "orihime computes in float32; dtype= is an FP32-accumulation "
...
class Operator:
    ...
    def _sensitivity_one(
        ...
        **config: Any,
        ...
        """Return the map sensitivity to one scalar scoring parameter."""

        primary = self._mask_primary(primary, mask)
        if not isinstance(wrt, str):
            raise TypeError("wrt must be a string")
        if wrt not in self.params:
            raise ValueError(
                f"invalid sensitivity field {wrt!r}; expected one of "
                f"{self.params!r}"
            )
        tensor_inputs, params, config = self._split_call_inputs(
            primary, tuple(inputs_and_params), config, dtype, lengths
