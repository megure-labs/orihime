...
CKY is a score-native soft binary constituency parser. It is the only orihime
...
`ori.raw.cky.vjp_fields` contains `leaf_scores` and `temperature`. The stateful
layer is `ori.nn.CKY`.
