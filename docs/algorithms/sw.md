...
```python
affine_map = ori.sw_affine(
    pair_scores,
    gap_open_score=-1.0,
    gap_extend_score=-0.25,
    temperature=0.9,
)
affine_value = ori.sw_affine_value(
    pair_scores,
    gap_open_score=-1.0,
    gap_extend_score=-0.25,
    temperature=0.9,
...
Selected detached parameter VJPs are exposed by `ori.raw.sw` and
`ori.raw.sw_affine`.

```python
cotangent = torch.randn_like(alignment)
temperature_vjp = ori.raw.sw.vjp_one(
    pair_scores.detach(),
    wrt="temperature",
    cotangent=cotangent,
    gap_score=-0.7,
