...
Pass a scalar tensor directly, or use `ori.nn`.
...
`ori.nn.SmithWaterman(learnable=("gap_score",))` provides the stateful layer
equivalent.
...
minibatches before calling ori.
...
`[B, L1, L2, 2]`. Use
`ori.build_damerau_transposition_sources(source_tokens, target_tokens)` to
construct them.

## How do infinite masks work?

Pass a boolean `mask=` (`True` marks cells to exclude) to any map/value/entropy
function; orihime applies the orientation-correct infinity internally and normalizes
...
`ori.raw.<op>` exposes the named kernel bindings (`forward`, `forward_t`,
...
code should use the top-level functions or `ori.nn`.
