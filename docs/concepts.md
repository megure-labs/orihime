...
Orihime turns classic dynamic programs into differentiable PyTorch functions.
...
1. `ori.<op>(...)` returns the map.
2. `ori.<op>_value(...)` returns one soft DP value per batch item.
3. `ori.<op>_entropy(...)` returns one entropy per batch item.

The map has the same shape as the primary score or cost tensor. CKY returns a
merge map shaped like `merge_scores`; `ori.cky_leaf_map(...)` is a separate
...
Selected detached parameter VJPs are available under `ori.raw`; trainable
modules are available under `ori.nn`.
