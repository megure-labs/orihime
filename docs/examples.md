...
global_map = d2p.nw_affine(
    global_scores,
    gap_open_score=gap_open,
    gap_extend_score=gap_extend,
    temperature=1.0,
)
global_value = d2p.nw_affine_value(
    global_scores,
    gap_open_score=gap_open,
    gap_extend_score=gap_extend,
    temperature=1.0,
...
mas_value = d2p.mas_value(mas_scores, temperature=1.0)
...
arc_map = d2p.eisner(arc_scores, temperature=1.0)
dependency_value = d2p.eisner_value(
    arc_scores,
    temperature=1.0,
)
...
sources = d2p.build_damerau_transposition_sources(
    source_tokens,
    target_tokens,
)
damerau_value = d2p.damerau_value(
    damerau_costs,
    temperature=0.5,
...
temperature_vjp = d2p.raw.lev.vjp_one(
    substitution_costs,
    wrt="temperature",
    cotangent=cotangent,
    insertion_cost=1.0,
...
all_parameter_vjps = d2p.raw.lev.vjp(
    substitution_costs,
    wrt=d2p.raw.lev.vjp_fields,
    cotangent=cotangent,
    insertion_cost=1.0,
    deletion_cost=1.0,
    temperature=0.5,
...
assert set(all_parameter_vjps) == set(d2p.raw.lev.vjp_fields)
```
