...
- **Constrain when you can**: `orihime.dtw(..., bandwidth=w)` (Sakoe-Chiba band) prunes the transitions considered and rules out pathological warps (memory still `O(L1·L2)`).
...
- **Parsers are cubic**: `orihime.cky`/`orihime.eisner` are `O(N³)`; keep `N` modest or budget accordingly.
