...
orihime's v3 surface consists of plain, keyword-oriented functions. Each of the
...
| --- | --- | --- | --- |
| local alignment | `ori.sw` | `gap_score`, `temperature` | `lengths` |
| affine local alignment | `ori.sw_affine` | `gap_open_score`, `gap_extend_score`, `temperature` | `lengths` |
| global alignment | `ori.nw` | `gap_score`, `temperature` | `lengths` |
| affine global alignment | `ori.nw_affine` | `gap_open_score`, `gap_extend_score`, `temperature` | `lengths` |
| DTW | `ori.dtw` | `temperature` | `lengths`, `bandwidth` |
| LCS | `ori.lcs` | `temperature` | `lengths` |
| Levenshtein | `ori.lev` | `insertion_cost`, `deletion_cost`, `temperature` | `lengths` |
| OSA | `ori.osa` | `insertion_cost`, `deletion_cost`, `transposition_cost`, `temperature` | `lengths`, `allowed_transpositions` |
| Damerau | `ori.damerau` | `insertion_cost`, `deletion_cost`, `transposition_cost`, `temperature` | `lengths`, `transposition_sources` |
| MAS | `ori.mas` | `temperature` | `lengths` |
| CKY | `ori.cky` | `temperature` | none |
| Eisner | `ori.eisner` | `temperature` | `lengths` |
...
`ori.raw.<op>` exposes detached, type-stable VJPs for the fields listed by
...
`ori.build_damerau_transposition_sources(...)`.
...
- keep tensor-valued scalars on the same device and in FP32 with the primary
  input;
- use `ori.nn` constructor `device=` and `dtype=` when a module should manage
  those scalars.
...
map/value/entropy function; orihime applies the orientation-correct infinity
...
`ori.raw.<op>` exposes named wrappers around the compiled primitives (the same
module that provides the VJPs), and `torch.ops.orihime.*` is the dispatcher floor.
...
schema order. Prefer
the high-level functions or `ori.nn` unless direct primitive control is required.
