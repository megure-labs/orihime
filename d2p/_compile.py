...
for _name in (
    ...
    register_fake(f"d2p::{_name}_forward")(_named_forward_fake)
    register_fake(f"d2p::{_name}_forward_t")(_named_forward_fake)


for _name in ("dtw", "mas", "eisner", "lcs"):
    register_fake(f"d2p::{_name}_value_grad_params")(_named_batch_fake)
...
for _name, _count in {
    ...
    register_fake(f"d2p::{_name}_value_grad_params")(
        _named_batch_tuple_fake(_count)
    )


register_fake("d2p::cky_value_grad_params")(_cky_value_grad_params_fake)
...
for _name, _param_count in {
    ...
    register_fake(f"d2p::{_name}_marginals_backward")(
        _named_map_backward_fake(_param_count)
    )


register_fake("d2p::cky_marginals_backward")(
...
for _name in (
    ...
    register_fake(f"d2p::{_name}_marginals_hvp")(_named_map_fake)
...
for _name in (
    ...
    register_fake(f"d2p::{_name}")(_named_map_fake)
...
try:
    ...
    # d2p/ops/*.py. Keep every forward, backward, HVP, and sensitivity
    # primitive on the same FP32 autocast policy.
    for _op_name in _SHIPPED_OPS:
        register_autocast(f"d2p::{_op_name}", "cuda", torch.float32)
