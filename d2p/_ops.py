...
For most users, the high-level ``d2p.<op>`` Operator instances are
recommended. Public low-level access is available through ``d2p.raw.<op>``;
...
def _has_cuda_dispatch():
    try:
        return torch._C._dispatch_has_kernel_for_dispatch_key("d2p::sw_forward", "CUDA")
...
def _load_extension():
    ...
    if libs:
        try:
            ...
            # The common cause is `pip install d2p` building from source under
            ...
            if "undefined symbol" in str(exc):
                raise ImportError(
                    f"d2p's compiled extension is not compatible with the "
                    ...
                    f"    pip install --no-build-isolation --no-cache-dir py-d2p\n\n"
...
def _ensure_cuda_supported(args, kwargs):
    ...
    if _contains_cuda_tensor(args) or _contains_cuda_tensor(kwargs):
        raise RuntimeError(
            "This d2p installation was built without CUDA support. "
...
def _vmap_restore_batch(tensor, vmap_batch, native_batch, op_name):
    ...
    if tensor.ndim == 0 or tensor.shape[0] != expected_batch:
        raise RuntimeError(
            f"d2p::{op_name} returned shape {tuple(tensor.shape)} from a "
...
def _make_batch_folding_vmap_rule(
    ...
    raw = getattr(torch.ops.d2p, op_name).default
    ...
    sensitivity_ops = tuple(
        getattr(torch.ops.d2p, name).default
    ...
    def vmap_rule(info, in_dims, *args):
        for index, in_dim in enumerate(in_dims):
            if in_dim is not None and index not in batch_indices:
                raise RuntimeError(
                    f"d2p::{op_name} does not support mapping scalar "
        ...
        for index in batch_indices:
            ...
            if native_batch is None:
                ...
                raise RuntimeError(
                    f"d2p::{op_name} received inconsistent native batch "
        ...
        if native_batch is None:
            raise RuntimeError(
                f"d2p::{op_name} requires a mapped batch-aligned tensor"
            )
        ...
        if batch_output_count < 1:
            raise RuntimeError(
                f"d2p::{op_name} vmap metadata does not match its outputs"
            )
        restored = [
            _vmap_restore_batch(
                output,
    ...
    for family, param_fields in _VMAP_FAMILY_PARAM_FIELDS.items():
        ...
        for op_name in primitive_names:
            torch.library.register_vmap(
                getattr(torch.ops.d2p, op_name).default,
        ...
        torch.library.register_vmap(
            getattr(torch.ops.d2p, backward_name).default,
