...
NN_CLASSES = tuple(
    getattr(orihime.nn, spec.nn_class) for spec in OPERATOR_CASES
)
...
DTYPE_ERROR = "orihime computes in float32.*only torch.float32 is supported"
...
def _call(
    ...
    *,
    ...
    target = getattr(orihime, f"{case.name}{suffix}")
...
def test_invalid_dynamic_temperature_raises_under_fullgraph_compile(
    ...
    inputs = _inputs(case, _device())
    structural = _structural_kwargs(case, inputs[0])
    target = getattr(orihime, case.name)
    fixed_params = {
        name: value
        for name, value in case.params
        if name != "temperature"
...
def test_numerical_domain_boundary_is_accepted():
    ...
    result = orihime.sw(scores, temperature=1.0)
...
def test_out_of_domain_dynamic_scores_raise_under_fullgraph_compile():
    torch._dynamo.reset()
    compiled = torch.compile(
        lambda scores: orihime.nw(scores, temperature=1.0),
        fullgraph=True,
        backend="aot_eager",
    )
...
def test_nn_compiled_mutated_temperature_raises():
    device = _device()
    layer = orihime.nn.NeedlemanWunsch(temperature=0.9, device=device)
...
def test_cky_and_eisner_validate_repeated_dimensions():
    ...
    with pytest.raises(ValueError, match="arc_scores dimension N"):
        orihime.eisner(torch.randn(1, 3, 4))
...
def test_pairwise_lengths_must_be_contiguous():
    ...
    with pytest.raises(ValueError, match="lengths must be contiguous"):
        orihime.sw(scores, lengths=lengths)
