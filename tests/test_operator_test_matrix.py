...
"""Executable drift gates for the shared fourteen-operator top-level matrix."""
...
def _assert_capabilities_are_meaningful(spec: Any) -> None:
    ...
    expected = {
        ...
        "score-alignment": spec.name
        in {"sw", "sw_affine", "sv_linear", "sv_affine", "nw", "nw_affine"},
        ...
        "saigo-vert": spec.name in {"sv_linear", "sv_affine"},
    }
...
def test_catalog_matches_the_authoritative_top_level_operator_surface() -> None:
    ...
    public_expected = tuple(
        "sv" if name == "sv_linear" else name for name in expected
    )

    assert expected == OPERATOR_NAMES
    assert tuple(ops.__all__) == public_expected
    assert orihime.ops is ops
    assert set(ops._kernels) == set(public_expected)
