...
def _control_root() -> Path:
    configured = os.environ.get("D2P_CONTROL_REPO_ROOT", "").strip()
    if not configured:
        return _source_root()
    candidate = Path(configured).expanduser().resolve()
    if not candidate.is_dir() or not (candidate / "pyproject.toml").is_file():
        raise AssertionError(
            "D2P_CONTROL_REPO_ROOT is configured but invalid: "
            f"{candidate}. Set D2P_CONTROL_REPO_ROOT to a valid d2p control "
...
def _operator_imports_from_helper() -> tuple[str, ...]:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.ImportFrom) or node.module != "d2p.ops":
            continue
        return tuple(alias.name for alias in node.names)
    raise AssertionError("operator_test_utils.py no longer imports d2p.ops")
...
def test_catalog_matches_the_authoritative_twelve_operator_surface() -> None:
    ...
    for case in OPERATOR_CASES:
        ...
        for entrypoint in case.public_entrypoints:
            assert hasattr(d2p, entrypoint), entrypoint
        assert hasattr(d2p.raw, case.name), case.name
        assert hasattr(d2p.nn, case.nn_class), case.nn_class
