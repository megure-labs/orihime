...
try:
    import orihime
    from orihime.ops import damerau as damerau_ops
...
class TestForward:
    ...
    def test_distance(self, batch_size, seq_lengths, temperature, cost_params, device):
        ...
        distance_orihime = damerau_ops.forward(sub_costs, trans_src, ins_cost, del_cost, trans_cost, temperature, None)[0]
    ...
    def test_distance_adjacent_transpositions(self, batch_size, temperature, device):
        ...
        assert allclose(distance_ref, distance_orihime), \
            f"Distance mismatch (adjacent): max diff = {max_diff(distance_ref, distance_orihime)}"
    ...
    def test_transposition_with_gaps(self, device):
        ...
        assert allclose(distance_ref, distance_orihime), \
            f"Distance mismatch (gaps): max diff = {max_diff(distance_ref, distance_orihime)}"
...
class TestBackward:
    ...
    def test_posteriors(self, batch_size, seq_lengths, temperature, cost_params, device):
        ...
        posteriors_orihime = damerau_ops.forward(sub_costs, trans_src, ins_cost, del_cost, trans_cost, temperature, None)[1]
...
class TestHVP:
    ...
    def test_hvp_finite_diff(self, device):
        ...
        hvp_orihime = damerau_ops.marginals_hvp(sub_costs, trans_src, V, ins_cost, del_cost, trans_cost, temperature, None)
...
@pytest.mark.skipif(
    ...
    reason="Damerau CUDA memory-safety regressions need the CUDA orihime backend",
)
