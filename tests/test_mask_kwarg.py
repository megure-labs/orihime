...
``d2p.<op>(x, mask=m)`` is identical to writing the orientation-correct infinity by hand.
"""
...
def test_mask_kwarg_equals_manual_infinity(op: str, observable: str) -> None:
    ...
    fn = getattr(d2p, f"{op}{observable}")
...
def test_cky_mask_kwarg_equals_manual_infinity(observable: str) -> None:
    ...
    mask[0, 0, 1, 3] = True
    fn = getattr(d2p, f"cky{observable}")
...
def test_mask_must_be_boolean() -> None:
    ...
    with pytest.raises(TypeError):
        d2p.sw(x, mask=torch.zeros(1, 4, 4, device=_DEVICE))  # float, not bool


def test_mask_shape_must_match() -> None:
    x = torch.randn(1, 4, 4, device=_DEVICE)
    with pytest.raises(ValueError):
        d2p.sw(x, mask=torch.zeros(1, 3, 3, dtype=torch.bool, device=_DEVICE))
