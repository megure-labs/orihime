...
if ORIHIME_AVAILABLE:
    from orihime.ops import lev as lev_ops
...
class TestForward:
    ...
    def test_distance(self, batch_size, seq_lengths, temperature, ins_del_costs, device):
        ...
        distance_orihime = lev_ops.forward(scores, ins_cost, del_cost, temperature, None)[0]
    ...
    def test_distance_symmetric(self, batch_size, temperature, device):
        ...
        assert allclose(distance_ref, distance_orihime), \
            f"Distance mismatch (symmetric): max diff = {max_diff(distance_ref, distance_orihime)}"
...
class TestBackward:
    ...
    def test_posteriors(self, batch_size, seq_lengths, temperature, ins_del_costs, device):
        ...
        posteriors_orihime = lev_ops.forward(scores, ins_cost, del_cost, temperature, None)[1]
...
class TestHVP:
    ...
    def test_hvp_finite_diff(self, device):
        ...
        hvp_orihime = lev_ops.marginals_hvp(scores, V, ins_cost, del_cost, temperature, None)
    ...
    def test_hvp_asymmetric_costs(self, device):
        ...
        assert allclose(hvp_fd, hvp_orihime, rtol=1e-2, atol=2e-3), \
            f"HVP mismatch (asymmetric): max diff = {max_diff(hvp_fd, hvp_orihime)}"
