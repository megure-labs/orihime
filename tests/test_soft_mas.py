...
try:
    import d2p
    from d2p.ops import mas as mas_ops
...
class TestForward:
    ...
    def test_score(self, batch_size, seq_lengths, temperature, device):
        ...
        score_d2p, _ = mas_forward_with_grads(scores, temperature, None)
...
class TestBackward:
    ...
    def test_posteriors(self, batch_size, seq_lengths, temperature, device):
        ...
        _, posteriors_d2p = mas_forward_with_grads(scores, temperature, None)
...
class TestHVP:
    ...
    def test_hvp_finite_diff(self, device):
        ...
        hvp_d2p = mas_ops.marginals_hvp(scores, V, temperature, None)
...
class TestLengthValidation:
    ...
    def test_public_mas_audit_repro_raises(self):
        ...
        with pytest.raises(RuntimeError, match=r"lengths\[0,0\] must be between 1 and 8"):
            d2p.mas(scores, temperature=1.0, lengths=lengths)
...
CPU_MAS_CALLS = {
    "mas": lambda scores, tangent: d2p.mas(
        scores,
        temperature=1.0,
    ),
...
def test_cuda_kernels_use_wide_cell_offsets_for_mas_indices():
    text = _source_text("src/mas/kernels.cu")
...
def test_cuda_wrappers_guard_mas_launch_device_and_validate_tangent():
    ...
    assert text.count("D2P_CUDA_GUARD(scores)") >= 6
...
def test_device_01_current_device_differs_for_all_public_and_raw_entrypoints():
    ...
    try:
        ...
        calls = (
            ("public_map", lambda: d2p.mas(scores, lengths=lengths)),
            ("public_value", lambda: d2p.mas_value(scores, lengths=lengths)),
            ("public_entropy", lambda: d2p.mas_entropy(scores, lengths=lengths)),
...
def test_device_02_wrong_device_secondary_tensors_are_rejected():
    ...
    try:
        ...
        wrong_lengths_calls = (
            ("public_map_lengths", lambda: d2p.mas(scores, lengths=other_lengths)),
            ("public_value_lengths", lambda: d2p.mas_value(scores, lengths=other_lengths)),
            ("public_entropy_lengths", lambda: d2p.mas_entropy(scores, lengths=other_lengths)),
        ...
        wrong_temperature_calls = (
            ("public_map_temperature", lambda: d2p.mas(
                scores,
                temperature=other_temperature,
                lengths=lengths,
            )),
            ("public_value_temperature", lambda: d2p.mas_value(
                scores,
                temperature=other_temperature,
                lengths=lengths,
            )),
            ("public_entropy_temperature", lambda: d2p.mas_entropy(
                scores,
                temperature=other_temperature,
                lengths=lengths,
            )),
...
def test_pub_01_mas_public_observables_are_finite_and_shaped():
    ...
    map_result = d2p.mas(scores, temperature=1.0)
    value_result = d2p.mas_value(scores, temperature=1.0)
    entropy_result = d2p.mas_entropy(scores, temperature=1.0)
...
def test_pub_02_mas_public_docstrings_state_score_orientation():
    ...
    for name, expected_fragment in expected_fragments.items():
        docstring = inspect.getdoc(getattr(d2p, name))
...
def test_edge_01_fam_06_mas_feasibility_edges_are_explicit():
    with pytest.raises(ValueError, match=r"scores\.shape\[-2\] >= scores\.shape\[-1\]"):
        d2p.mas(torch.randn(1, 1, 2), temperature=1.0)

    scores = torch.randn(1, 4, 3)
    infeasible_lengths = torch.tensor([[2, 3]], dtype=torch.int32)
    for function in (d2p.mas, d2p.mas_value, d2p.mas_entropy):
        with pytest.raises(
            (RuntimeError, ValueError),
...
def test_guard_02_noncontiguous_primary_and_private_derivatives_are_initialized():
    ...
    cases = (
        (
            "public_map",
            lambda s, v: d2p.mas(s, temperature=0.9, lengths=lengths),
        ),
        (
            "public_value",
            lambda s, v: d2p.mas_value(s, temperature=0.9, lengths=lengths),
        ),
        (
            "public_entropy",
            lambda s, v: d2p.mas_entropy(s, temperature=0.9, lengths=lengths),
        ),
        (
            "raw_forward",
...
def test_over_02_cuda_wide_index_source_contract_is_explicit():
    kernel = _source_text("src/mas/kernels.cu")
    ...
    assert wrapper.count("D2P_CUDA_GUARD(scores)") >= 6
...
def test_over_02_public_cuda_empty_batch_is_rejected_before_dispatch():
    ...
    with pytest.raises(ValueError, match=r"empty leading batch \(B=0\)"):
        d2p.mas(scores, temperature=1.0)
