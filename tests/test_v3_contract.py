...
def _raw_named_derivative_call(
    ...
    *,
    ...
    elif case.name in {"sw", "sw_affine", "nw", "nw_affine"}:
...
def test_bare_functions_have_frozen_signatures_and_shapes(
    ...
    for suffix in ("", "_value", "_entropy"):
        function = getattr(d2p, f"{case.name}{suffix}")
    ...
    inputs = _inputs(case)
    map_result = _map_function(case)(*inputs)
    value_result = getattr(d2p, f"{case.name}_value")(*inputs)
    entropy_result = getattr(d2p, f"{case.name}_entropy")(*inputs)
    ...
    if case.name == "cky":
        leaf_map = d2p.cky_leaf_map(*inputs)
...
def test_entropy_matches_signed_temperature_derivative_of_value(
    ...
    value_function = getattr(d2p, f"{case.name}_value")
    entropy_function = getattr(d2p, f"{case.name}_entropy")
    epsilon = 1.0e-2
...
def test_raw_vjp_tier_is_mandatory_selected_and_type_stable(
    ...
    # Collapsed low-level tier: d2p.raw.<op> re-exports the kernel bindings that
    # formerly lived under the internal d2p.ops.<op>, so d2p.raw is the single
    # low-level surface. Alias the internal module locally so it does not shadow
    # the module-level ``d2p`` referenced above.
    from d2p import ops as _ops_internal

    kernel_module = getattr(_ops_internal, case.name)
    for binding in kernel_module.__all__:
        assert hasattr(raw, binding), f"d2p.raw.{case.name} missing {binding}"
...
def test_nn_modules_register_frozen_parameters_as_buffers(
    ...
    module_class = getattr(d2p.nn, case.nn_class)
    layer = module_class()
...
def test_osa_structural_mask_contract() -> None:
    ...
    torch.testing.assert_close(
        d2p.osa(costs),
        d2p.osa(costs, allowed_transpositions=disabled),
    )

    enabled = disabled.clone()
    enabled[:, 1:, 1:] = True
    result = d2p.osa(costs, allowed_transpositions=enabled)
    ...
    with pytest.raises(ValueError, match="first row and first column"):
        d2p.osa(costs, allowed_transpositions=invalid_boundary)
...
def test_damerau_structural_sources_and_builder_contract() -> None:
    ...
    torch.testing.assert_close(
        d2p.damerau(costs),
        d2p.damerau(costs, transposition_sources=sentinel),
    )
    ...
    built = d2p.build_damerau_transposition_sources(
        source_tokens,
        target_tokens,
        lengths=lengths,
    )
...
def test_public_version_is_available():
    assert isinstance(d2p.__version__, str)
    assert d2p.__version__.startswith("0.1.0")
