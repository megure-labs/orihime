...
class TestForward:
    ...
    def test_partition(self, batch_size, seq_lengths, temperature, device):
        ...
        partition_orihime = sw_ops.forward(scores, gap, temperature, None)[0]
    ...
    def test_partition_large_batch_warp_dispatch(self):
        ...
        assert allclose(partition_ref, partition_orihime), \
            f"Warp partition mismatch: max diff = {max_diff(partition_ref, partition_orihime)}"
...
class TestBackward:
    ...
    def test_posteriors(self, batch_size, seq_lengths, temperature, device):
        ...
        posteriors_orihime = sw_ops.forward(scores, gap, temperature, None)[1]
    ...
    def test_posteriors_large_batch_warp_dispatch(self):
        ...
        assert allclose(posteriors_ref, posteriors_orihime, rtol=1e-3, atol=1e-4), \
            f"Warp posterior mismatch: max diff = {max_diff(posteriors_ref, posteriors_orihime)}"
    ...
    def test_temperature_gradient(self, device):
        ...
        grad_T_orihime = sw_forward_with_grads(scores, gap, temperature, None)[3]
...
class TestHVP:
    ...
    def test_hvp_finite_diff(self, device):
        ...
        hvp_orihime = sw_ops.marginals_hvp(scores, V, gap, temperature, None)
