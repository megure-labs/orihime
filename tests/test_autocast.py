...
from d2p.ops import sw_affine as sw_affine_ops
...
class TestAutocastSW:
    ...
    def test_sw_affine_autocast(self):
        ...
        with torch.autocast("cuda", dtype=torch.float16):
            value = d2p.sw_affine_value(
                scores,
                gap_open_score=-2.0,
                gap_extend_score=-0.5,
                temperature=1.0,
            )
            marginals = d2p.sw_affine(
                scores,
                gap_open_score=-2.0,
                gap_extend_score=-0.5,
                temperature=1.0,
...
# ---------------------------------------------------------------------------
# All-12-operator autocast map-backward-after-exit + MAS contracted backward.
# Self-contained 12-operator cases mirroring tests/test_compile.py, exercising
# the map autograd backward across the autocast boundary (FP16 inputs promoted
# to FP32 inside autocast; gradients restored to the FP16 input dtype after the
# context exits) and the MAS explicit contracted backward inside autocast.
# ---------------------------------------------------------------------------
...
def _autocast_tensor_args(case, device):
    ...
    if case.name == "damerau":
        ...
        trans_src = d2p.build_damerau_transposition_sources(
            source_tokens,
            target_tokens,
        )
...
def _autocast_call_value(case, args, param_values):
    op = getattr(d2p, f"{case.name}_value")
    params = {
        name: value
...
def test_mas_contracted_backward_in_autocast():
    ...
    temp = 0.9
    with torch.autocast("cuda", dtype=torch.float16):
        marginals = d2p.mas(scores, temperature=temp)
        grads = d2p.raw.mas.vjp(
            scores,
            temperature=temp,
            cotangent=torch.ones_like(marginals),
            wrt=d2p.raw.mas.vjp_fields,
        )
