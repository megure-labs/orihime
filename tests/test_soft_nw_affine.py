...
try:
    import d2p
    from d2p.ops import nw_affine as nw_affine_ops
...
class TestForward:
    ...
    def test_score(self, batch_size, seq_lengths, gap_penalties, temperature, device):
        ...
        score_d2p = nw_affine_ops.forward(scores, gap_open, gap_ext, temperature, None)[0]
...
class TestBackward:
    ...
    def test_posteriors(self, batch_size, seq_lengths, gap_penalties, temperature, device):
        ...
        posteriors_d2p = nw_affine_ops.forward(scores, gap_open, gap_ext, temperature, None)[1]
...
class TestHVP:
    ...
    def test_hvp_finite_diff(self, device):
        ...
        hvp_d2p = nw_affine_ops.marginals_hvp(scores, V, gap_open, gap_ext, temperature, None)
    ...
    def test_hvp_various_gaps(self, device):
        ...
        for gap_open, gap_ext in [(-1.0, -0.3), (-2.0, -0.5), (-3.0, -1.0)]:
            ...
            assert allclose(hvp_fd, hvp_d2p, rtol=1e-2, atol=2e-3), \
                f"HVP mismatch for gap_open={gap_open}, gap_ext={gap_ext}: max diff = {max_diff(hvp_fd, hvp_d2p)}"
...
def test_tensor_parameters_use_scores_device_when_current_device_differs():
    ...
    try:
        ...
        value_result = d2p.nw_affine_value(
            scores,
            gap_open_score=gap_open,
            gap_extend_score=gap_ext,
            temperature=temperature,
...
def test_tensor_parameters_reject_wrong_cuda_device(parameter_name):
    ...
    with pytest.raises(
        ...
        d2p.nw_affine(scores, lengths=lengths, **kwargs)
...
_NW_AFFINE_CUDA_REASON = "nw_affine CUDA memsafety regressions require a CUDA d2p build"


@pytest.mark.skipif(not (D2P_AVAILABLE and CUDA_AVAILABLE), reason=_NW_AFFINE_CUDA_REASON)
...
def test_cuda_entries_run_on_scores_device_when_current_device_differs(entry):
    ...
    try:
        ...
        if entry == "nw_affine_forward":
            ...
            result = d2p.ops.nw_affine.marginals_hvp(
                scores, tangent, GAP_OPEN, GAP_EXT, TEMPERATURE, lengths
            )
        elif entry == "nw_affine_marginals_backward":
            result = d2p.ops.nw_affine.marginals_backward(
                scores, grad_alignment, GAP_OPEN, GAP_EXT, TEMPERATURE, lengths
            )
        else:
