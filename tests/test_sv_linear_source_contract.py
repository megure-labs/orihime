...
CUDA = ROOT / "src" / "sv_linear" / "kernels_gpu.cu"
HIP = ROOT / "src" / "sv_linear" / "kernels_gpu.hip"
CPU = ROOT / "src" / "sv_linear" / "kernels_cpu.cpp"
CUDA_HEADER = ROOT / "src" / "sv_linear" / "kernels_gpu.cuh"
HIP_HEADER = ROOT / "src" / "sv_linear" / "kernels_gpu.hiph"
...
def test_hip_source_has_the_complete_linear_launch_surface():
    hip = HIP.read_text()
    gpu_hosts = re.findall(r"void\s+(sv_linear_\w+)\s*\(", HIP_HEADER.read_text())
    device_symbols = re.findall(r"__global__\s+void\s+(sv_linear_\w+)\s*\(", hip)
    assert gpu_hosts == [
        "sv_linear_forward",
        "sv_linear_backward",
        "sv_linear_hvp",
    ...
    assert "sv_linear_param_grad_init_U_kernel" not in hip
    assert all(hip.count(f"{symbol}<<<") >= 1 for symbol in device_symbols)
