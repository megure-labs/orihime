...
/**
 * @file torch_hip.cpp
 * @brief Soft Eisner HIP PyTorch Bindings
 *
...
void validate_eisner_arc_scores_hip(const torch::Tensor& arc_scores) {
...
void validate_eisner_lengths_hip(
...
torch::Tensor resolve_eisner_lengths_hip(
    ...
    validate_eisner_lengths_hip(lengths, B, n, device);
    return lengths;
}

void validate_eisner_tangent_hip(
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_eisner_arc_scores_hip(arc_scores);
        ...
        torch::Tensor lengths = resolve_eisner_lengths_hip(lengths_opt, B, n, arc_scores.device());
    ...
    static torch::autograd::tensor_list backward(
        ...
        if (grad_marginals.defined() && grad_marginals.numel() > 0) {
            ...
            // CPU and HIP return the same complete temperature gradient.
...
std::vector<torch::Tensor> soft_eisner_hip(
    torch::Tensor arc_scores,
    torch::Tensor temperature,
...
torch::Tensor soft_eisner_float_hip(
    torch::Tensor arc_scores,
    double temperature,
    c10::optional<torch::Tensor> lengths
) {
...
std::tuple<torch::Tensor, torch::Tensor> soft_eisner_with_grads_hip(
    torch::Tensor arc_scores,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt
) {
...
torch::Tensor soft_eisner_hvp_hip(
    torch::Tensor arc_scores,
    torch::Tensor V,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt
    ...
    validate_eisner_arc_scores_hip(arc_scores);
    validate_eisner_tangent_hip(arc_scores, V);
    ORIHIME_CUDA_GUARD(arc_scores);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> soft_eisner_backward_full_hip(
    torch::Tensor arc_scores,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt
) {
...
torch::Tensor soft_eisner_param_jacobian_hip(
    torch::Tensor arc_scores,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt
) {
...
std::vector<torch::Tensor> eisner_forward_hip(
    torch::Tensor arc_scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    ...
    return soft_eisner_hip(arc_scores, temp_t, lengths);
}

// eisner::forward_t - tensor parameter version
std::vector<torch::Tensor> eisner_forward_t_hip(
    torch::Tensor arc_scores,
    torch::Tensor temp,
    torch::Tensor lengths
) {
    return soft_eisner_hip(arc_scores, temp, lengths);
}

// eisner::value_grad_params - returns grad_temp per batch
torch::Tensor eisner_value_grad_params_hip(
    torch::Tensor arc_scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    auto result = soft_eisner_backward_full_hip(arc_scores, temp, lengths);
    return std::get<2>(result);
}

// eisner::marginals_backward - full backward through marginals
std::tuple<torch::Tensor, torch::Tensor> eisner_marginals_backward_hip(
    torch::Tensor arc_scores,
    ...
    torch::Tensor grad_arc_scores =
        soft_eisner_hvp_hip(arc_scores, grad_marginals, temp, lengths);
    torch::Tensor dP_dT =
        soft_eisner_param_jacobian_hip(arc_scores, temp, lengths);
...
torch::Tensor eisner_marginals_hvp_hip(
    torch::Tensor arc_scores,
    torch::Tensor v,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_eisner_hvp_hip(arc_scores, v, temp, lengths);
}

// eisner::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor eisner_marginals_grad_temp_hip(
    torch::Tensor arc_scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_eisner_param_jacobian_hip(arc_scores, temp, lengths);
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_eisner", soft_eisner_hip);
    m.impl("soft_eisner_float", soft_eisner_float_hip);
    m.impl("soft_eisner_with_grads", soft_eisner_with_grads_hip);
    m.impl("soft_eisner_hvp", soft_eisner_hvp_hip);
    m.impl("soft_eisner_backward_full", soft_eisner_backward_full_hip);
    m.impl("soft_eisner_param_jacobian", soft_eisner_param_jacobian_hip);

    // Namespaced API
    m.impl("eisner_forward", eisner_forward_hip);
    m.impl("eisner_forward_t", eisner_forward_t_hip);
    m.impl("eisner_value_grad_params", eisner_value_grad_params_hip);
    m.impl("eisner_marginals_backward", eisner_marginals_backward_hip);
    m.impl("eisner_marginals_hvp", eisner_marginals_hvp_hip);
    m.impl("eisner_marginals_grad_temp", eisner_marginals_grad_temp_hip);
}
