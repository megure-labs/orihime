...
This document describes the modular architecture for ORIHIME operators and serves as the pattern for adding new operators.
...
7. **Add Python API**: `orihime/<op>.py` and `orihime/ops/<op>.py`
...
Each module registers to the same `orihime` namespace, which PyTorch merges at runtime.
