...
/**
 * @file torch_hip.cpp
 * @brief Soft MAS HIP PyTorch Bindings
 *
...
void validate_mas_scores_hip(const torch::Tensor& scores) {
...
void validate_mas_temperature_tensor_hip(
    const torch::Tensor& scores,
...
torch::Tensor resolve_mas_lengths_hip(
    ...
    validate_mas_lengths_hip(lengths, B, max_T, max_S, device);
    return lengths;
}
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_mas_scores_hip(scores);
        ...
        torch::Tensor lengths = resolve_mas_lengths_hip(
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_mas_scores_hip(scores);
        validate_mas_temperature_tensor_hip(scores, temperature);
        ORIHIME_CUDA_GUARD(scores);
...
torch::Tensor soft_mas_hip(
    ...
    torch::Tensor lens = resolve_mas_lengths_hip(
        lengths, B, max_T, max_S, scores_contig.device()
    );
...
std::vector<torch::Tensor> soft_mas_hip_float(
    torch::Tensor scores,
    double temperature,
    c10::optional<torch::Tensor> lengths
) {
...
std::tuple<torch::Tensor, torch::Tensor> soft_mas_hip_with_grads(
    torch::Tensor scores,
    double temperature,
    c10::optional<torch::Tensor> lengths
) {
...
torch::Tensor soft_mas_hvp_hip(
    ...
    validate_mas_scores_hip(scores);
    validate_mas_tangent_hip(scores, V);
    ORIHIME_CUDA_GUARD(scores);
...
torch::Tensor soft_mas_param_jacobian_hip(
    torch::Tensor scores,
    double temperature,
    c10::optional<torch::Tensor> lengths
) {
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> soft_mas_backward_full_hip(
    torch::Tensor scores,
    double temperature,
    c10::optional<torch::Tensor> lengths
) {
...
std::vector<torch::Tensor> mas_forward_hip(
    torch::Tensor scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_mas_hip_float(scores, temp, lengths);
}

std::vector<torch::Tensor> mas_forward_t_hip(
    torch::Tensor scores,
    torch::Tensor temp,
    torch::Tensor lengths
) {
...
torch::Tensor mas_value_grad_params_hip(
    torch::Tensor scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    auto result = soft_mas_backward_full_hip(scores, temp, lengths);
    return std::get<2>(result);
}

std::tuple<torch::Tensor, torch::Tensor> mas_marginals_backward_hip(
    torch::Tensor scores,
    ...
    validate_mas_scores_hip(scores);
    validate_mas_tangent_hip(scores, grad_marginals);
    ORIHIME_CUDA_GUARD(scores);
...
torch::Tensor mas_marginals_hvp_hip(
    torch::Tensor scores,
    torch::Tensor v,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_mas_hvp_hip(scores, v, temp, lengths);
}

torch::Tensor mas_marginals_grad_temp_hip(
    torch::Tensor scores,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_mas_param_jacobian_hip(scores, temp, lengths);
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_mas", soft_mas_hip);
    m.impl("soft_mas_float", soft_mas_hip_float);
    m.impl("soft_mas_with_grads", soft_mas_hip_with_grads);
    m.impl("soft_mas_hvp", soft_mas_hvp_hip);
    m.impl("soft_mas_param_jacobian", soft_mas_param_jacobian_hip);
    m.impl("soft_mas_backward_full", soft_mas_backward_full_hip);

    m.impl("mas_forward", mas_forward_hip);
    m.impl("mas_forward_t", mas_forward_t_hip);
    m.impl("mas_value_grad_params", mas_value_grad_params_hip);
    m.impl("mas_marginals_backward", mas_marginals_backward_hip);
    m.impl("mas_marginals_hvp", mas_marginals_hvp_hip);
    m.impl("mas_marginals_grad_temp", mas_marginals_grad_temp_hip);
}
