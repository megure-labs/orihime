...
/**
 * @file torch_hip.cpp
 * @brief HIP PyTorch bindings for Affine Gap Smith-Waterman
 *
 * Provides GPU-accelerated affine gap SW with autograd support.
 * Three-state DP (Match, Insert, Delete) with separate gap open/extend penalties.
 *
 * Operators registered:
 *   - soft_sw_affine: Tensor params for full differentiability
 *   - soft_sw_affine_float: Float params (convenience)
 *   - soft_sw_affine_with_grads: Forward + backward in one call
 *   - soft_sw_affine_hvp: Hessian-vector product
 *   - soft_sw_affine_param_jacobian: Parameter gradients
 *   - soft_sw_affine_backward_full: Full backward pass
 *   - sw_affine_* namespace: Clean API wrappers
 */
...
int checked_sw_affine_size_to_int(int64_t value, const char* name) {
...
int64_t checked_sw_affine_alpha_size(int max_L1, int max_L2) {
    TORCH_CHECK(
        max_L1 <= std::numeric_limits<int>::max() - max_L2,
        "sw_affine dimensions too large: L1 + L2 exceeds CUDA int index range"
    );
    ...
    TORCH_CHECK(
        ...
        "sw_affine DP table too large: (L1 + 1) * (L2 + 1) overflows size_t"
    );
    const size_t cells = rows * cols;
    TORCH_CHECK(
        cells <= std::numeric_limits<size_t>::max() / 3,
        "sw_affine DP table too large: 3 * (L1 + 1) * (L2 + 1) overflows size_t"
    );
    const size_t alpha_size = 3 * cells;
    TORCH_CHECK(
        alpha_size <= static_cast<size_t>(std::numeric_limits<int>::max()),
        "sw_affine DP table too large: 3 * (L1 + 1) * (L2 + 1) exceeds CUDA int index range"
    );
    return static_cast<int64_t>(alpha_size);
}

void validate_sw_affine_lengths_hip(
...
torch::Tensor resolve_sw_affine_lengths_hip(
    ...
    validate_sw_affine_lengths_hip(lengths, B, max_L1, max_L2, device);
    return lengths;
}
...
class SoftSWAffineCUDAFunction : public torch::autograd::Function<SoftSWAffineCUDAFunction> {
public:
    static torch::autograd::tensor_list forward(
        ...
        int B = checked_sw_affine_size_to_int(scores.size(0), "scores.size(0)");
        int max_L1 = checked_sw_affine_size_to_int(scores.size(1), "scores.size(1)");
        int max_L2 = checked_sw_affine_size_to_int(scores.size(2), "scores.size(2)");
        int64_t alpha_size = checked_sw_affine_alpha_size(max_L1, max_L2);

        validate_sw_affine_lengths_hip(lengths, B, max_L1, max_L2, scores.device());
        ...
        orihime::common::record_streams_current({&alpha, &scores, &partition, &beta, &posteriors, &grad_open, &grad_ext, &grad_T, &lengths});
        sw_affine_backward_hip(
            alpha.data_ptr<float>(),
            scores.data_ptr<float>(),
            partition.data_ptr<float>(),
            beta.data_ptr<float>(),
    ...
    static torch::autograd::tensor_list backward(
        ...
        if (grad_score.defined() && grad_score.numel() > 0) {
            ...
            orihime::common::record_streams_current({&alpha, &scores, &partition, &beta, &posteriors, &tmp_open, &tmp_ext, &tmp_T, &lengths});
            sw_affine_backward_hip(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
        ...
        if (grad_posteriors.defined() && grad_posteriors.numel() > 0) {
            ...
            orihime::common::record_streams_current({&alpha, &scores, &partition, &grad_posteriors, &d_alpha, &d_partition, &hvp_beta_ws, &d_beta, &hvp_grad_scores, &lengths});
            sw_affine_hvp_hip(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
            ...
            // dP/dgap_open
            orihime::common::record_streams_current({&alpha, &scores, &partition, &grad_open_fwd, &U_ws, &beta_ws, &W_ws, &dP_dtheta, &lengths});
            sw_affine_param_grad_hip(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
            ...
            // dP/dgap_ext
            orihime::common::record_streams_current({&alpha, &scores, &partition, &grad_ext_fwd, &U_ws, &beta_ws, &W_ws, &dP_dtheta, &lengths});
            sw_affine_param_grad_hip(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
...
std::vector<torch::Tensor> soft_sw_affine_hip(
    torch::Tensor scores,
    torch::Tensor gap_open,
    torch::Tensor gap_ext,
    torch::Tensor temperature,
    ...
    ORIHIME_CHECK_INPUT_CUDA(scores);
    ORIHIME_CUDA_GUARD(scores);
    return SoftSWAffineCUDAFunction::apply(scores, gap_open, gap_ext, temperature, lengths);
}

// Affine SW with float params (convenience function)
// Returns (score, alignment) - both differentiable
std::vector<torch::Tensor> soft_sw_affine_hip_float(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temperature,
    ...
    int B = checked_sw_affine_size_to_int(scores.size(0), "scores.size(0)");
    int L1 = checked_sw_affine_size_to_int(scores.size(1), "scores.size(1)");
    int L2 = checked_sw_affine_size_to_int(scores.size(2), "scores.size(2)");
    ...
    torch::Tensor lengths = resolve_sw_affine_lengths_hip(
        lengths_opt, B, L1, L2, scores.device()
    );

    return SoftSWAffineCUDAFunction::apply(scores, gap_open_t, gap_ext_t, temp_t, lengths);
}

// Affine SW with explicit gradients
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
soft_sw_affine_hip_with_grads(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temperature,
...
// Affine SW HVP
torch::Tensor soft_sw_affine_hvp_hip(
    torch::Tensor scores,
    ...
    orihime::common::record_streams_current({&alpha, &scores, &partition, &tangent, &d_alpha, &d_partition, &hvp_beta_ws, &d_beta, &H_scores, &lengths});
    sw_affine_hvp_hip(
        alpha.data_ptr<float>(),
        scores.data_ptr<float>(),
        partition.data_ptr<float>(),
...
torch::Tensor soft_sw_affine_param_jacobian_hip(
    torch::Tensor scores,
...
soft_sw_affine_backward_full_hip(
    torch::Tensor scores,
    ...
    orihime::common::record_streams_current({&alpha, &scores, &partition, &beta_fwd, &posteriors, &grad_open_fwd, &grad_ext_fwd, &grad_T_fwd, &lengths});
    sw_affine_backward_hip(
    ...
    orihime::common::record_streams_current({&alpha, &scores, &partition, &grad_alignment, &d_alpha, &d_partition, &hvp_beta_ws, &d_beta, &grad_scores, &lengths});
    sw_affine_hvp_hip(
        alpha.data_ptr<float>(), scores.data_ptr<float>(),
...
// =============================================================================
// Namespaced API (sw_affine::*)
// These are thin wrappers around existing functions with cleaner names
// =============================================================================

// sw_affine::forward - returns (value, marginals)
std::vector<torch::Tensor> orihime_sw_affine_forward_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sw_affine_hip_float(scores, gap_open, gap_ext, temp, lengths);
}

// sw_affine::forward_t - tensor params version
std::vector<torch::Tensor> orihime_sw_affine_forward_t_hip(
    torch::Tensor scores,
    torch::Tensor gap_open,
    torch::Tensor gap_ext,
    torch::Tensor temp,
    ...
    return soft_sw_affine_hip(scores, gap_open, gap_ext, temp, lengths);
}

// sw_affine::value_grad_params - returns (grad_gap_open, grad_gap_ext, grad_temp) per batch
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> orihime_sw_affine_value_grad_params_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    auto result = soft_sw_affine_hip_with_grads(scores, gap_open, gap_ext, temp, lengths);
    return std::make_tuple(std::get<2>(result), std::get<3>(result), std::get<4>(result));
}

// sw_affine::marginals_backward - full backward through marginals
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> orihime_sw_affine_marginals_backward_hip(
    torch::Tensor scores,
    ...
    return soft_sw_affine_backward_full_hip(scores, grad_marginals, gap_open, gap_ext, temp, lengths);
}

// sw_affine::marginals_hvp - Hessian-vector product
torch::Tensor orihime_sw_affine_marginals_hvp_hip(
    torch::Tensor scores,
    torch::Tensor v,
    double gap_open,
    double gap_ext,
    ...
    return soft_sw_affine_hvp_hip(scores, v, gap_open, gap_ext, temp, lengths);
}

// sw_affine::marginals_grad_gap_open - d(marginals)/d(gap_open)
torch::Tensor orihime_sw_affine_marginals_grad_gap_open_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sw_affine_param_jacobian_hip(scores, 0, gap_open, gap_ext, temp, lengths);
}

// sw_affine::marginals_grad_gap_ext - d(marginals)/d(gap_ext)
torch::Tensor orihime_sw_affine_marginals_grad_gap_ext_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sw_affine_param_jacobian_hip(scores, 1, gap_open, gap_ext, temp, lengths);
}

// sw_affine::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor orihime_sw_affine_marginals_grad_temp_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sw_affine_param_jacobian_hip(scores, 2, gap_open, gap_ext, temp, lengths);
}
...
// Register HIP implementations through PyTorch's CUDA dispatch key for affine SW operators
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    // Core affine SW operators
    m.impl("soft_sw_affine", soft_sw_affine_hip);
    m.impl("soft_sw_affine_float", soft_sw_affine_hip_float);
    m.impl("soft_sw_affine_with_grads", soft_sw_affine_hip_with_grads);
    m.impl("soft_sw_affine_hvp", soft_sw_affine_hvp_hip);
    m.impl("soft_sw_affine_param_jacobian", soft_sw_affine_param_jacobian_hip);
    m.impl("soft_sw_affine_backward_full", soft_sw_affine_backward_full_hip);

    // sw_affine namespace
    m.impl("sw_affine_forward", orihime_sw_affine_forward_hip);
    m.impl("sw_affine_forward_t", orihime_sw_affine_forward_t_hip);
    m.impl("sw_affine_value_grad_params", orihime_sw_affine_value_grad_params_hip);
    m.impl("sw_affine_marginals_backward", orihime_sw_affine_marginals_backward_hip);
    m.impl("sw_affine_marginals_hvp", orihime_sw_affine_marginals_hvp_hip);
    m.impl("sw_affine_marginals_grad_gap_open", orihime_sw_affine_marginals_grad_gap_open_hip);
    m.impl("sw_affine_marginals_grad_gap_ext", orihime_sw_affine_marginals_grad_gap_ext_hip);
    m.impl("sw_affine_marginals_grad_temp", orihime_sw_affine_marginals_grad_temp_hip);
}
...
TORCH_LIBRARY_IMPL(orihime, AutogradCUDA, m) {
    ...
    // sw_affine namespace - autograd versions
    m.impl("sw_affine_forward", orihime_sw_affine_forward_hip);
    m.impl("sw_affine_forward_t", orihime_sw_affine_forward_t_hip);
}
