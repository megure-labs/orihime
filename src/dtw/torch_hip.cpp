...
/**
 * @file torch_hip.cpp
 * @brief Soft DTW HIP Extension with PyTorch Autograd
 *
...
torch::Tensor resolve_dtw_lengths_hip(
    ...
    validate_dtw_lengths_hip(lengths, B, max_L1, max_L2, device);
    return lengths;
}
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_dtw_lengths_hip(lengths, B, max_L1, max_L2, costs.device());
...
std::vector<torch::Tensor> soft_dtw_hip(
    torch::Tensor costs,
    torch::Tensor temperature,
    torch::Tensor lengths,
    int64_t bandwidth
...
std::vector<torch::Tensor> soft_dtw_hip_float(
    torch::Tensor costs,
    double temperature,
    ...
    torch::Tensor lengths = resolve_dtw_lengths_hip(
...
torch::Tensor soft_dtw_param_jacobian_hip(
    torch::Tensor costs,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt,
    c10::optional<int64_t> bandwidth_opt
...
std::vector<torch::Tensor> dtw_forward_hip_wrapper(
    torch::Tensor costs,
    double temp,
    c10::optional<torch::Tensor> lengths,
    c10::optional<int64_t> bandwidth
) {
    return soft_dtw_hip_float(costs, temp, lengths, bandwidth);
}

// dtw::forward_t - tensor temperature version
std::vector<torch::Tensor> dtw_forward_t_hip(
    torch::Tensor costs,
    torch::Tensor temp,
    torch::Tensor lengths,
    int64_t bandwidth
) {
    return soft_dtw_hip(costs, temp, lengths, bandwidth);
}

// dtw::value_grad_params - returns grad_temp per batch
torch::Tensor dtw_value_grad_params_hip(
    torch::Tensor costs,
    double temp,
    c10::optional<torch::Tensor> lengths,
    c10::optional<int64_t> bandwidth
) {
    auto result = soft_dtw_hip_with_grads(costs, temp, lengths, bandwidth);
    return std::get<2>(result);
}

// dtw::marginals_backward - full backward through marginals
std::tuple<torch::Tensor, torch::Tensor> dtw_marginals_backward_hip(
    torch::Tensor costs,
    ...
    return soft_dtw_backward_full_hip(costs, grad_marginals, temp, lengths, bandwidth);
}

// dtw::marginals_hvp - Hessian-vector product
torch::Tensor dtw_marginals_hvp_hip(
    torch::Tensor costs,
    torch::Tensor v,
    double temp,
    c10::optional<torch::Tensor> lengths,
    ...
    return soft_dtw_hvp_hip(costs, v, temp, lengths, bandwidth);
}

// dtw::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor dtw_marginals_grad_temp_hip(
    torch::Tensor costs,
    double temp,
    c10::optional<torch::Tensor> lengths,
    c10::optional<int64_t> bandwidth
) {
    return soft_dtw_param_jacobian_hip(costs, temp, lengths, bandwidth);
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_dtw", soft_dtw_hip);
    m.impl("soft_dtw_float", soft_dtw_hip_float);
    m.impl("soft_dtw_with_grads", soft_dtw_hip_with_grads);
    m.impl("soft_dtw_hvp", soft_dtw_hvp_hip);
    m.impl("soft_dtw_param_jacobian", soft_dtw_param_jacobian_hip);
    m.impl("soft_dtw_backward_full", soft_dtw_backward_full_hip);

    // Namespaced API
    m.impl("dtw_forward", dtw_forward_hip_wrapper);
    m.impl("dtw_forward_t", dtw_forward_t_hip);
    m.impl("dtw_value_grad_params", dtw_value_grad_params_hip);
    m.impl("dtw_marginals_backward", dtw_marginals_backward_hip);
    m.impl("dtw_marginals_hvp", dtw_marginals_hvp_hip);
    m.impl("dtw_marginals_grad_temp", dtw_marginals_grad_temp_hip);
}
