...
// soft_sv_linear_cpu.cpp
//
// CPU implementation of canonical Saigo-Vert linear-gap alignment.
...
//   - ParamGrad: Cross-derivatives dP/dtheta for gap and temperature
//
// Shapes:
...
// ============================================================================
// Saigo-Vert-specific constants and helper functions
// ============================================================================
...
extern "C" void sv_linear_forward_cpu(
        ...
        for (int64_t batch = begin; batch < end; ++batch) {
            ...
            for (int k = 2; k <= L1 + L2; k++) {
                ...
                for (int i = i_start; i <= i_end; i++) {
                    ...
                    // Can come from: M[i-1,j] + gap, I[i-1,j] + gap
                    float i_from_M = (i > 1) ? a[STATE_M * cell_stride + cell_up] + gap : NINF;
                    float i_from_I = (i > 1) ? a[STATE_I * cell_stride + cell_up] + gap : NINF;

                    a[STATE_I * cell_stride + cell_idx] = logsumexp2(i_from_M, i_from_I, T);

                    // D state: gap in seq1 (moving right, j increases)
                    // SV canonical (Saigo-Vert): D can come from M+gap, D+gap,
                    // AND I+gap -- the one-way I->D cross transition. This makes each
                    ...
                    float d_from_M = (j > 1) ? a[STATE_M * cell_stride + cell_left] + gap : NINF;
                    float d_from_I = (j > 1) ? a[STATE_I * cell_stride + cell_left] + gap : NINF;
                    float d_from_D = (j > 1) ? a[STATE_D * cell_stride + cell_left] + gap : NINF;
...
extern "C" void sv_linear_backward_cpu(
    ...
    at::parallel_for(0, B, 1, [&](int64_t begin, int64_t end) {
        for (int64_t batch = begin; batch < end; ++batch) {
                    ...
                    if (!std::isinf(beta_I) && !std::isnan(beta_I) && beta_I != 0.0f) {
                        ...
                        if (i > 1) {
                            if (wI_M > 0.0f) {
                                ...
                                sum_gap_grad.add(beta_I * wI_M);
                            }
                            if (wI_I > 0.0f) {
                                be[STATE_I * cell_stride + cell_up] += beta_I * wI_I;
                                sum_gap_grad.add(beta_I * wI_I);
                            }
                        }
                    }
                    ...
                    if (!std::isinf(beta_D) && !std::isnan(beta_D) && beta_D != 0.0f) {
                        ...
                        if (j > 1) {
                            if (wD_M > 0.0f) {
                                ...
                                sum_gap_grad.add(beta_D * wD_M);
                            }
                            if (wD_I > 0.0f) {
                                be[STATE_I * cell_stride + cell_left] += beta_D * wD_I;
                                sum_gap_grad.add(beta_D * wD_I);  // canonical I->D cross
                            }
                            if (wD_D > 0.0f) {
                                be[STATE_D * cell_stride + cell_left] += beta_D * wD_D;
                                sum_gap_grad.add(beta_D * wD_D);
                            }
                        }
                    }
                }
            ...
            float gap_count = sum_gap_grad.result();
            float expected_gap_cost = gap_count * gap;
            KahanSum expected_total_sum;
            expected_total_sum.add(match_score_sum.result());
            expected_total_sum.add(expected_gap_cost);

            grad_gap[b] = gap_count;
            grad_T[b] = (S - expected_total_sum.result()) / T;
        }
    });
}
...
extern "C" void sv_linear_hvp_cpu(
    const float* alpha,       // [B, 3*(L1+1)*(L2+1)]
    const float* scores,      // [B, L1, L2]
    const float* partition,   // [B]
...
enum LinearParamType {
    PARAM_GAP = 0,
    PARAM_TEMPERATURE = 1
};

extern "C" void sv_linear_param_grad_cpu(
    const float* alpha,       // [B, 3*(L1+1)*(L2+1)]
    const float* scores,      // [B, L1, L2]
    const float* partition,   // [B]
        ...
        for (int64_t batch = begin; batch < end; ++batch) {
            ...
            for (int k = 2; k <= L1 + L2; k++) {
                ...
                for (int i = i_start; i <= i_end; i++) {
                    ...
                    if (param_type == PARAM_GAP) {
                        du_D_M += 1.0f;
                        du_D_I += 1.0f;  // canonical I->D cross
                        du_D_D += 1.0f;
                    }
            ...
            for (int k = L1 + L2; k >= 2; k--) {
                ...
                for (int i = i_start; i <= i_end; i++) {
                    ...
                    if (i > 1) {
                        float i_from_M = a[STATE_M * cell_stride + cell_up] + gap;
                        float i_from_I = a[STATE_I * cell_stride + cell_up] + gap;
                        float wI_M, wI_I;
                        softmax2_weights_cpu(i_from_M, i_from_I, T, wI_M, wI_I);
                    ...
                    if (j > 1) {
                        float d_from_M = a[STATE_M * cell_stride + cell_left] + gap;
                        float d_from_I = a[STATE_I * cell_stride + cell_left] + gap;
                        float d_from_D = a[STATE_D * cell_stride + cell_left] + gap;
                        float wD_M, wD_I, wD_D;
                        softmax3_weights_cpu(d_from_M, d_from_I, d_from_D, T, wD_M, wD_I, wD_D);
