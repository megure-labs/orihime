...
/**
 * @file torch_cuda.cpp
 * @brief Canonical Saigo-Vert linear-gap CUDA Extension with PyTorch Autograd
 *
...
#include "sv_linear/kernels.cuh"

using namespace d2p::common;

namespace {

struct SvLinearCudaShape {
    int B;
    int max_L1;
    int max_L2;
    int64_t alpha_size;
...
int checked_sv_linear_cuda_dim(int64_t value, const char* name) {
    TORCH_CHECK(
...
SvLinearCudaShape checked_sv_linear_cuda_shape(const torch::Tensor& scores) {
    TORCH_CHECK(scores.dim() == 3, "scores must be 3D (B, L1, L2)");
    int B = checked_sv_linear_cuda_dim(scores.size(0), "scores batch dimension");
    int max_L1 = checked_sv_linear_cuda_dim(scores.size(1), "scores L1 dimension");
    int max_L2 = checked_sv_linear_cuda_dim(scores.size(2), "scores L2 dimension");
    ...
    TORCH_CHECK(
        ...
        "SV linear CUDA DP table is too large for 32-bit kernel indexing: ",
...
void validate_sv_linear_lengths_cuda(
...
torch::Tensor resolve_sv_linear_lengths_cuda(
    ...
    validate_sv_linear_lengths_cuda(lengths, B, max_L1, max_L2, device);
    return lengths;
}
...
class SoftSVLinearCUDAFunction : public torch::autograd::Function<SoftSVLinearCUDAFunction> {
public:
    static torch::autograd::tensor_list forward(
        ...
        SvLinearCudaShape shape = checked_sv_linear_cuda_shape(scores);
        int B = shape.B;
        int max_L1 = shape.max_L1;
        int max_L2 = shape.max_L2;
        int64_t alpha_size = shape.alpha_size;

        validate_sv_linear_lengths_cuda(lengths, B, max_L1, max_L2, scores.device());
        ...
        d2p::common::record_streams_current({&alpha, &scores, &partition, &beta, &posteriors, &grad_gap, &grad_T, &lengths});
        sv_linear_backward(
            alpha.data_ptr<float>(),
            scores.data_ptr<float>(),
            partition.data_ptr<float>(),
    ...
    static torch::autograd::tensor_list backward(
        ...
        if (grad_score.defined() && grad_score.numel() > 0) {
            ...
            d2p::common::record_streams_current({&alpha, &scores, &partition, &beta, &posteriors, &tmp_gap, &tmp_T, &lengths});
            sv_linear_backward(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
        ...
        if (grad_posteriors.defined() && grad_posteriors.numel() > 0) {
            ...
            d2p::common::record_streams_current({&alpha, &scores, &partition, &grad_posteriors, &d_alpha, &d_partition, &beta, &d_beta, &hvp_grad_scores, &lengths});
            sv_linear_hvp(
                alpha.data_ptr<float>(),
                scores.data_ptr<float>(),
                partition.data_ptr<float>(),
            ...
            // dP/dgap
            d2p::common::record_streams_current({&alpha, &scores, &partition, &grad_gap_fwd, &U_ws, &beta_ws, &W_ws, &dP_dtheta, &lengths});
...
std::vector<torch::Tensor> soft_sv_linear_cuda(
    torch::Tensor scores,
    torch::Tensor gap,
    torch::Tensor temperature,
    torch::Tensor lengths
) {
    return SoftSVLinearCUDAFunction::apply(scores, gap, temperature, lengths);
}

// Saigo-Vert linear with float params (convenience function)
std::vector<torch::Tensor> soft_sv_linear_cuda_float(
    torch::Tensor scores,
    double gap,
    double temperature,
    ...
    torch::Tensor lengths = resolve_sv_linear_lengths_cuda(lengths_opt, B, L1, L2, scores.device());

    return SoftSVLinearCUDAFunction::apply(scores, gap_t, temp_t, lengths);
}

// Saigo-Vert linear with explicit gradients (for debugging/inspection)
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
soft_sv_linear_cuda_with_grads(
    torch::Tensor scores,
    double gap,
    double temperature,
    c10::optional<torch::Tensor> lengths_opt
    ...
    auto options = scores.options();
    torch::Tensor lengths = resolve_sv_linear_lengths_cuda(lengths_opt, B, max_L1, max_L2, options.device());
...
torch::Tensor soft_sv_linear_hvp_cuda(
    torch::Tensor scores,
    ...
    d2p::common::record_streams_current({&alpha, &scores, &partition, &tangent, &d_alpha, &d_partition, &beta, &d_beta, &H_scores, &lengths});
    sv_linear_hvp(
        alpha.data_ptr<float>(),
        scores.data_ptr<float>(),
        partition.data_ptr<float>(),
...
torch::Tensor soft_sv_linear_param_jacobian_cuda(
    torch::Tensor scores,
...
soft_sv_linear_backward_full_cuda(
    torch::Tensor scores,
    ...
    d2p::common::record_streams_current({&alpha, &scores, &partition, &beta_fwd, &posteriors, &grad_gap_fwd, &grad_T_fwd, &lengths});
    sv_linear_backward(
    ...
    d2p::common::record_streams_current({&alpha, &scores, &partition, &grad_alignment, &d_alpha, &d_partition, &beta, &d_beta, &grad_scores, &lengths});
    sv_linear_hvp(
        alpha.data_ptr<float>(), scores.data_ptr<float>(),
...
std::vector<torch::Tensor> d2p_sv_linear_forward_cuda(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_sv_linear_cuda_float(scores, gap, temp, lengths);
}

// sv_linear::forward_t - tensor params version
std::vector<torch::Tensor> d2p_sv_linear_forward_t_cuda(
    torch::Tensor scores,
    torch::Tensor gap,
    torch::Tensor temp,
    torch::Tensor lengths
) {
    return soft_sv_linear_cuda(scores, gap, temp, lengths);
}

// sv_linear::value_grad_params - returns (grad_gap, grad_temp) per batch
std::tuple<torch::Tensor, torch::Tensor> d2p_sv_linear_value_grad_params_cuda(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    auto result = soft_sv_linear_cuda_with_grads(scores, gap, temp, lengths);
    return std::make_tuple(std::get<2>(result), std::get<3>(result));
}

// sv_linear::marginals_backward - full backward through marginals
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> d2p_sv_linear_marginals_backward_cuda(
    torch::Tensor scores,
    ...
    return soft_sv_linear_backward_full_cuda(scores, grad_marginals, gap, temp, lengths);
}

// sv_linear::marginals_hvp - Hessian-vector product
torch::Tensor d2p_sv_linear_marginals_hvp_cuda(
    torch::Tensor scores,
    torch::Tensor v,
    double gap,
    double temp,
    ...
    return soft_sv_linear_hvp_cuda(scores, v, gap, temp, lengths);
}

// sv_linear::marginals_grad_gap - d(marginals)/d(gap)
torch::Tensor d2p_sv_linear_marginals_grad_gap_cuda(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_sv_linear_param_jacobian_cuda(scores, 0, gap, temp, lengths);
}

// sv_linear::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor d2p_sv_linear_marginals_grad_temp_cuda(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_sv_linear_param_jacobian_cuda(scores, 1, gap, temp, lengths);
}
...
TORCH_LIBRARY_IMPL(d2p, CUDA, m) {
    m.impl("soft_sv_linear", soft_sv_linear_cuda);
    m.impl("soft_sv_linear_float", soft_sv_linear_cuda_float);
    m.impl("soft_sv_linear_with_grads", soft_sv_linear_cuda_with_grads);
    m.impl("soft_sv_linear_hvp", soft_sv_linear_hvp_cuda);
    m.impl("soft_sv_linear_param_jacobian", soft_sv_linear_param_jacobian_cuda);
    m.impl("soft_sv_linear_backward_full", soft_sv_linear_backward_full_cuda);

    // Namespaced API
    m.impl("sv_linear_forward", d2p_sv_linear_forward_cuda);
    m.impl("sv_linear_forward_t", d2p_sv_linear_forward_t_cuda);
    m.impl("sv_linear_value_grad_params", d2p_sv_linear_value_grad_params_cuda);
    m.impl("sv_linear_marginals_backward", d2p_sv_linear_marginals_backward_cuda);
    m.impl("sv_linear_marginals_hvp", d2p_sv_linear_marginals_hvp_cuda);
    m.impl("sv_linear_marginals_grad_gap", d2p_sv_linear_marginals_grad_gap_cuda);
    m.impl("sv_linear_marginals_grad_temp", d2p_sv_linear_marginals_grad_temp_cuda);
}
