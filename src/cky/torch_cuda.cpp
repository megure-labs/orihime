...
void validate_cky_chart_shapes_cuda(
...
void validate_cky_temperature_tensor_cuda(
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_cky_chart_shapes_cuda(merge_scores, leaf_scores);

        torch::Tensor temperature_contig = temperature.contiguous();
        validate_cky_temperature_tensor_cuda(temperature_contig, B, n, merge_scores.device());
...
std::vector<torch::Tensor> cky_forward_cuda(
    ...
    return ::soft_cky_float_cuda(merge_scores, leaf_scores, temp);
}
...
std::vector<torch::Tensor> cky_forward_t_cuda(
    ...
    return ::soft_cky_cuda(merge_scores, leaf_scores, temp);
}
...
std::tuple<torch::Tensor, torch::Tensor> cky_value_grad_params_cuda(
    ...
    auto result = ::soft_cky_with_grads_cuda(
...
cky_marginals_backward_cuda(
    ...
    return ::soft_cky_backward_full_cuda(
...
torch::Tensor cky_marginals_grad_temp_cuda(
    ...
    return ::soft_cky_param_jacobian_cuda(
        merge_scores, leaf_scores, temp
    );
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    ...
    // Namespaced API
    m.impl("cky_forward", orihime::cky::cky_forward_cuda);
    m.impl("cky_forward_t", orihime::cky::cky_forward_t_cuda);
    m.impl("cky_value_grad_params", orihime::cky::cky_value_grad_params_cuda);
    m.impl("cky_marginals_backward", orihime::cky::cky_marginals_backward_cuda);
    m.impl("cky_marginals_hvp", orihime::cky::cky_marginals_hvp_cuda);
    m.impl("cky_marginals_grad_leaf", orihime::cky::cky_marginals_grad_leaf_cuda);
    m.impl("cky_marginals_grad_temp", orihime::cky::cky_marginals_grad_temp_cuda);
}
