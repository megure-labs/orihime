...
public:
    static torch::autograd::tensor_list forward(
        ...
        ORIHIME_CHECK_INPUT_CPU(merge_scores);
        ORIHIME_CHECK_INPUT_CPU(leaf_scores);
        ORIHIME_CHECK_INPUT_CPU(temperature);
...
torch::Tensor soft_cky_hvp_cpu(
    ...
    ORIHIME_CHECK_INPUT_CPU(merge_scores);
    ORIHIME_CHECK_INPUT_CPU(leaf_scores);
    ORIHIME_CHECK_INPUT_CPU(v_merge);
    ORIHIME_CHECK_INPUT_CPU(v_leaf);
    validate_cky_chart_shapes_cpu(merge_scores, leaf_scores);
...
soft_cky_backward_full_cpu(
    ...
    ORIHIME_CHECK_INPUT_CPU(merge_scores);
    ORIHIME_CHECK_INPUT_CPU(leaf_scores);
    ORIHIME_CHECK_INPUT_CPU(grad_posteriors);
    validate_cky_chart_shapes_cpu(merge_scores, leaf_scores);
...
TORCH_LIBRARY_IMPL(orihime, CPU, m) {
    ...
    // Namespaced API
    m.impl("cky_forward", orihime::cky::cky_forward_cpu);
    m.impl("cky_forward_t", orihime::cky::cky_forward_t_cpu);
    m.impl("cky_value_grad_params", orihime::cky::cky_value_grad_params_cpu);
    m.impl("cky_marginals_backward", orihime::cky::cky_marginals_backward_cpu);
    m.impl("cky_marginals_hvp", orihime::cky::cky_marginals_hvp_cpu);
    m.impl("cky_marginals_grad_leaf", orihime::cky::cky_marginals_grad_leaf_cpu);
    m.impl("cky_marginals_grad_temp", orihime::cky::cky_marginals_grad_temp_cpu);
}
