...
// torch_utils.h - PyTorch validation macros and helpers for orihime
//
...
// See reduce.cuh for ORIHIME_STATIC_CHECK_BLOCK_SIZE (compile-time)
// ============================================================================

#define ORIHIME_CHECK_BLOCK_SIZE(block_dim) \
...
#define ORIHIME_CHECK_CONTIGUOUS(x) \
    TORCH_CHECK((x).is_contiguous(), #x " must be contiguous")

#define ORIHIME_CHECK_DIM(x, d) \
    TORCH_CHECK((x).dim() == (d), #x " must be " #d "D, got ", (x).dim(), "D")

// Dtype checks - current: float32 only
// NOTE: When adding FP16/BF16/FP64 support, replace ORIHIME_CHECK_FLOAT with
// ORIHIME_CHECK_FLOATING and update the error message accordingly.
#define ORIHIME_CHECK_FLOAT(x) \
    TORCH_CHECK((x).scalar_type() == torch::kFloat32, \
        #x " must be float32, got ", (x).scalar_type())

// Future: accept all supported floating types
// #define ORIHIME_CHECK_FLOATING(x) \
...
#define ORIHIME_CHECK_CUDA(x) \
    TORCH_CHECK((x).is_cuda(), #x " must be a CUDA tensor")

#define ORIHIME_CHECK_CPU(x) \
    TORCH_CHECK(!(x).is_cuda(), #x " must be a CPU tensor")

// Combined checks - use these at function entry
#define ORIHIME_CHECK_INPUT_CUDA(x) \
    ORIHIME_CHECK_CUDA(x); ORIHIME_CHECK_CONTIGUOUS(x); ORIHIME_CHECK_FLOAT(x)

#define ORIHIME_CHECK_INPUT_CPU(x) \
    ORIHIME_CHECK_CPU(x); ORIHIME_CHECK_CONTIGUOUS(x); ORIHIME_CHECK_FLOAT(x)
...
#define ORIHIME_CHECK_LENGTHS_2D(lengths, B, device) \
...
#define ORIHIME_CHECK_LENGTHS_1D(lengths, B, device) \
    TORCH_CHECK((lengths).scalar_type() == torch::kInt32, \
        "lengths must be int32, got ", (lengths).scalar_type()); \
