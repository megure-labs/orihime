...
#define D2P_NUMERICS_HD __host__ __device__
#else
#define D2P_NUMERICS_HD
#endif
...
D2P_NUMERICS_HD inline float safe_exp(float x) {
...
struct KahanSum {
    ...
    D2P_NUMERICS_HD KahanSum() {}

    D2P_NUMERICS_HD inline void add(float val) {
        float y = val - c;
        float t = sum + y;
        c = (t - sum) - y;
        sum = t;
    ...
    D2P_NUMERICS_HD inline float result() const { return sum; }

    D2P_NUMERICS_HD inline void reset() {
        sum = 0.0f;
        c = 0.0f;
    }
};
...
D2P_NUMERICS_HD inline float logsumexp2(float a, float b, float temp) {
#if defined(__CUDA_ARCH__)
...
D2P_NUMERICS_HD inline float logsumexp3(float a, float b, float c, float temp) {
#if defined(__CUDA_ARCH__)
...
D2P_NUMERICS_HD inline float logsumexp4(float a, float b, float c, float d, float temp) {
#if defined(__CUDA_ARCH__)
...
D2P_NUMERICS_HD inline float softmin3(float a, float b, float c, float temp) {
#if defined(__CUDA_ARCH__)
