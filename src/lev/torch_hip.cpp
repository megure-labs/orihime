...
/**
 * @file torch_hip.cpp
 * @brief Soft Levenshtein HIP PyTorch Bindings
 *
 * HIP implementations are registered through PyTorch's CUDA dispatch key with
 * TORCH_LIBRARY_IMPL.
 */
...
int checked_lev_dim_to_int_hip(int64_t value, const char* name) {
    TORCH_CHECK(
...
int64_t checked_lev_alpha_size_hip(int64_t max_L1, int64_t max_L2) {
...
void validate_lev_scores_hip(const torch::Tensor& scores) {
    CHECK_INPUT(scores);
    TORCH_CHECK(scores.dim() == 3, "scores must be 3D (B, L1, L2)");
    TORCH_CHECK(scores.dtype() == torch::kFloat32, "scores must be float32");
    checked_lev_dim_to_int_hip(scores.size(0), "scores.size(0)");
    checked_lev_dim_to_int_hip(scores.size(1), "scores.size(1)");
    checked_lev_dim_to_int_hip(scores.size(2), "scores.size(2)");
    checked_lev_alpha_size_hip(scores.size(1), scores.size(2));
}

void validate_lev_lengths_hip(
...
torch::Tensor resolve_lev_lengths_hip(
    ...
    validate_lev_lengths_hip(lengths, B, max_L1, max_L2, options.device());
    return lengths;
}

void validate_lev_backward_input_hip(const torch::Tensor& scores, const torch::Tensor& grad_posteriors) {
...
// ============================================================================
// Levenshtein HIP Autograd Function
// ============================================================================
...
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_lev_scores_hip(scores);
        ...
        const int B = checked_lev_dim_to_int_hip(scores.size(0), "scores.size(0)");
        const int max_L1 = checked_lev_dim_to_int_hip(scores.size(1), "scores.size(1)");
        const int max_L2 = checked_lev_dim_to_int_hip(scores.size(2), "scores.size(2)");
        const int64_t alpha_size = checked_lev_alpha_size_hip(max_L1, max_L2);

        validate_lev_lengths_hip(lengths, B, max_L1, max_L2, scores.device());
...
static std::vector<torch::Tensor> soft_levenshtein_hip(
    torch::Tensor scores,
...
static std::vector<torch::Tensor> soft_levenshtein_hip_float(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temperature,
    ...
    const int B = checked_lev_dim_to_int_hip(scores.size(0), "scores.size(0)");
    const int L1 = checked_lev_dim_to_int_hip(scores.size(1), "scores.size(1)");
    const int L2 = checked_lev_dim_to_int_hip(scores.size(2), "scores.size(2)");
    ...
    torch::Tensor lengths = resolve_lev_lengths_hip(lengths_opt, B, L1, L2, options);
...
soft_levenshtein_hip_with_grads(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temperature,
    ...
    auto options = scores.options();
    torch::Tensor lengths = resolve_lev_lengths_hip(lengths_opt, B, max_L1, max_L2, options);
...
static torch::Tensor soft_levenshtein_hvp_hip(
    torch::Tensor scores,
...
static torch::Tensor soft_levenshtein_param_jacobian_hip(
    torch::Tensor scores,
    int64_t param_type,
    double ins_cost,
    double del_cost,
...
soft_levenshtein_backward_full_hip(
    torch::Tensor scores,
    ...
    validate_lev_scores_hip(scores);
    validate_lev_backward_input_hip(scores, grad_posteriors);
    ORIHIME_CUDA_GUARD(scores);
...
std::vector<torch::Tensor> lev_forward_hip_wrapper(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temp,
    ...
    return soft_levenshtein_hip_float(scores, ins_cost, del_cost, temp, lengths);
}

std::vector<torch::Tensor> lev_forward_t_hip(
    torch::Tensor scores,
    torch::Tensor ins_cost,
    torch::Tensor del_cost,
    torch::Tensor temp,
    ...
    return soft_levenshtein_hip(scores, ins_cost, del_cost, temp, lengths);
}

std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>
lev_value_grad_params_hip(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temp,
    ...
    auto result = soft_levenshtein_hip_with_grads(
...
lev_marginals_backward_hip(
    ...
    return soft_levenshtein_backward_full_hip(
        scores, grad_marginals, ins_cost, del_cost, temp, lengths
    );
}

torch::Tensor lev_marginals_hvp_hip(
    torch::Tensor scores,
    torch::Tensor v,
    double ins_cost,
    double del_cost,
    ...
    return soft_levenshtein_hvp_hip(
        scores, v, ins_cost, del_cost, temp, lengths
    );
}

torch::Tensor lev_marginals_grad_ins_cost_hip(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temp,
    ...
    return soft_levenshtein_param_jacobian_hip(
        scores, LEV_PARAM_INS, ins_cost, del_cost, temp, lengths
    );
}

torch::Tensor lev_marginals_grad_del_cost_hip(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temp,
...
torch::Tensor lev_marginals_grad_temp_hip(
    torch::Tensor scores,
    double ins_cost,
    double del_cost,
    double temp,
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_levenshtein", orihime::lev::soft_levenshtein_hip);
    m.impl("soft_levenshtein_float", orihime::lev::soft_levenshtein_hip_float);
    m.impl("soft_levenshtein_with_grads", orihime::lev::soft_levenshtein_hip_with_grads);
    m.impl("soft_levenshtein_hvp", orihime::lev::soft_levenshtein_hvp_hip);
    m.impl("soft_levenshtein_param_jacobian", orihime::lev::soft_levenshtein_param_jacobian_hip);
    m.impl("soft_levenshtein_backward_full", orihime::lev::soft_levenshtein_backward_full_hip);

    m.impl("lev_forward", orihime::lev::lev_forward_hip_wrapper);
    m.impl("lev_forward_t", orihime::lev::lev_forward_t_hip);
    m.impl("lev_value_grad_params", orihime::lev::lev_value_grad_params_hip);
    m.impl("lev_marginals_backward", orihime::lev::lev_marginals_backward_hip);
    m.impl("lev_marginals_hvp", orihime::lev::lev_marginals_hvp_hip);
    m.impl("lev_marginals_grad_ins_cost", orihime::lev::lev_marginals_grad_ins_cost_hip);
    m.impl("lev_marginals_grad_del_cost", orihime::lev::lev_marginals_grad_del_cost_hip);
    m.impl("lev_marginals_grad_temp", orihime::lev::lev_marginals_grad_temp_hip);
}
