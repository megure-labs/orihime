...
/**
 * @file torch_hip.cpp
 * @brief Soft OSA HIP PyTorch Bindings
 *
 * Provides torch.ops.orihime.soft_osa* operators for ROCm tensors through
...
void validate_osa_inputs_hip(const torch::Tensor& sub_costs, const torch::Tensor& trans_mask) {
...
void validate_osa_same_shape_hip(
...
torch::Tensor resolve_osa_lengths_hip(
    ...
    validate_osa_lengths_hip(lengths, B, L1, L2, device);
...
public:
    static torch::autograd::variable_list forward(
        ...
        // Function and the Levenshtein HIP Function) so backward returns their
        ...
        validate_osa_inputs_hip(sub_costs, trans_mask);
        ...
        auto lengths_t = resolve_osa_lengths_hip(lengths, B, L1, L2, sub_costs.device());
        ...
        // Forward pass
        orihime::common::record_streams_current({&sub_costs_c, &trans_mask_c, &alpha, &osa_score, &lengths_t});
        orihime::osa::osa_forward_hip(
        ...
        orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &beta, &posteriors, &grad_T, &grad_ins, &grad_del, &grad_trans, &lengths_t});
        orihime::osa::osa_backward_hip(
            alpha.data_ptr<float>(),
            sub_costs_c.data_ptr<float>(),
            trans_mask_c.data_ptr<float>(),
            osa_score.data_ptr<float>(),
    ...
    static torch::autograd::variable_list backward(
        ...
        // Function and the Levenshtein HIP backward so a loss through the distance/score
        ...
        if (grad_osa_score.defined() && grad_osa_score.numel() > 0) {
            ...
            orihime::common::record_streams_current({&alpha, &sub_costs, &trans_mask, &osa_score, &beta, &posteriors, &tmp_T, &tmp_ins, &tmp_del, &tmp_trans, &lengths_t});
            orihime::osa::osa_backward_hip(
                alpha.data_ptr<float>(),
        ...
        if (grad_posteriors.defined() && grad_posteriors.numel() > 0) {
            validate_osa_same_shape_hip(sub_costs, grad_posteriors, "grad_posteriors");
            ...
            orihime::common::record_streams_current({&alpha, &sub_costs, &trans_mask, &osa_score, &grad_posteriors, &d_alpha, &d_osa_score, &beta, &d_beta, &hvp_grad_sub_costs, &lengths_t});
            orihime::osa::osa_hvp_hip(
                alpha.data_ptr<float>(),
                sub_costs.data_ptr<float>(),
                trans_mask.data_ptr<float>(),
                osa_score.data_ptr<float>(),
            ...
            auto param_grad_term = [&](int param_type) -> torch::Tensor {
                ...
                orihime::common::record_streams_current({&alpha, &sub_costs, &trans_mask, &osa_score, &U, &beta_p, &W, &dP_dparam, &lengths_t});
                orihime::osa::osa_param_grad_hip(
                    alpha.data_ptr<float>(),
                    sub_costs.data_ptr<float>(),
                    trans_mask.data_ptr<float>(),
                    osa_score.data_ptr<float>(),
...
std::vector<torch::Tensor> soft_osa_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    torch::Tensor ins_cost_t,
    torch::Tensor del_cost_t,
...
std::vector<torch::Tensor> soft_osa_float_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    double ins_cost,
    double del_cost,
...
soft_osa_with_grads_hip(
    ...
    orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &beta, &posteriors, &grad_T, &grad_ins_out, &grad_del_out, &grad_trans_out, &lengths_t});
    orihime::osa::osa_backward_hip(
        alpha.data_ptr<float>(),
        sub_costs_c.data_ptr<float>(),
        trans_mask_c.data_ptr<float>(),
...
torch::Tensor soft_osa_hvp_hip(
    ...
    validate_osa_inputs_hip(sub_costs, trans_mask);
    validate_osa_same_shape_hip(sub_costs, V, "V");
    ...
    orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &V_c, &d_alpha, &d_osa_score, &beta, &d_beta, &H_scores, &lengths_t});
    orihime::osa::osa_hvp_hip(
        alpha.data_ptr<float>(),
        sub_costs_c.data_ptr<float>(),
        trans_mask_c.data_ptr<float>(),
...
torch::Tensor soft_osa_param_jacobian_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    int64_t param_type,
    double ins_cost,
    ...
    orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &U, &beta, &W, &dP_dparam, &lengths_t});
    orihime::osa::osa_param_grad_hip(
        alpha.data_ptr<float>(),
        sub_costs_c.data_ptr<float>(),
        trans_mask_c.data_ptr<float>(),
...
soft_osa_backward_full_hip(
    ...
    validate_osa_inputs_hip(sub_costs, trans_mask);
    validate_osa_same_shape_hip(sub_costs, grad_output, "grad_output");
    ...
    orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &grad_c, &d_alpha, &d_osa_score, &beta, &d_beta, &grad_sub_costs, &lengths_t});
    orihime::osa::osa_hvp_hip(
        alpha.data_ptr<float>(),
        sub_costs_c.data_ptr<float>(),
        trans_mask_c.data_ptr<float>(),
    ...
    orihime::common::record_streams_current({&alpha, &sub_costs_c, &trans_mask_c, &osa_score, &beta2, &posteriors, &grad_T, &grad_ins_out, &grad_del_out, &grad_trans_out, &lengths_t});
    orihime::osa::osa_backward_hip(
        alpha.data_ptr<float>(),
        sub_costs_c.data_ptr<float>(),
        trans_mask_c.data_ptr<float>(),
...
std::vector<torch::Tensor> osa_forward_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    double ins_cost,
    double del_cost,
...
osa_value_grad_params_hip(
    ...
    auto result = soft_osa_with_grads_hip(
        sub_costs, trans_mask, ins_cost, del_cost, trans_cost, temp, lengths
    );
...
osa_marginals_backward_hip(
    ...
    auto grad_sub_costs = soft_osa_hvp_hip(
        sub_costs,
        trans_mask,
        grad_marginals_c,
        ins_cost,
    ...
    auto grad_ins_jacobian = soft_osa_param_jacobian_hip(
        sub_costs, trans_mask, 0, ins_cost, del_cost, trans_cost, temp, lengths
    );
    auto grad_del_jacobian = soft_osa_param_jacobian_hip(
        sub_costs, trans_mask, 1, ins_cost, del_cost, trans_cost, temp, lengths
    );
    auto grad_trans_jacobian = soft_osa_param_jacobian_hip(
        sub_costs, trans_mask, 2, ins_cost, del_cost, trans_cost, temp, lengths
    );
    auto grad_temp_jacobian = soft_osa_param_jacobian_hip(
...
torch::Tensor osa_marginals_hvp_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    torch::Tensor v,
    double ins_cost,
...
torch::Tensor osa_marginals_grad_ins_cost_hip(
    ...
    return soft_osa_param_jacobian_hip(
        sub_costs, trans_mask, 0, ins_cost, del_cost, trans_cost, temp, lengths
    );
}

// osa::marginals_grad_del_cost - d(marginals)/d(del_cost)
torch::Tensor osa_marginals_grad_del_cost_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    double ins_cost,
    double del_cost,
...
torch::Tensor osa_marginals_grad_trans_cost_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    double ins_cost,
    double del_cost,
...
torch::Tensor osa_marginals_grad_temp_hip(
    torch::Tensor sub_costs,
    torch::Tensor trans_mask,
    double ins_cost,
    double del_cost,
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_osa", soft_osa_hip);
    m.impl("soft_osa_float", soft_osa_float_hip);
    m.impl("soft_osa_with_grads", soft_osa_with_grads_hip);
    m.impl("soft_osa_hvp", soft_osa_hvp_hip);
    m.impl("soft_osa_param_jacobian", soft_osa_param_jacobian_hip);
    m.impl("soft_osa_backward_full", soft_osa_backward_full_hip);

    // Namespaced API
    m.impl("osa_forward", osa_forward_hip);
    m.impl("osa_forward_t", osa_forward_t_hip);
    m.impl("osa_value_grad_params", osa_value_grad_params_hip);
    m.impl("osa_marginals_backward", osa_marginals_backward_hip);
    m.impl("osa_marginals_hvp", osa_marginals_hvp_hip);
    m.impl("osa_marginals_grad_ins_cost", osa_marginals_grad_ins_cost_hip);
    m.impl("osa_marginals_grad_del_cost", osa_marginals_grad_del_cost_hip);
    m.impl("osa_marginals_grad_trans_cost", osa_marginals_grad_trans_cost_hip);
    m.impl("osa_marginals_grad_temp", osa_marginals_grad_temp_hip);
}
