...
/**
 * @file torch_hip.cpp
 * @brief HIP PyTorch bindings for canonical Saigo-Vert local alignment
 *
...
int checked_sv_affine_size_to_int(int64_t value, const char* name) {
    ...
    TORCH_CHECK(
        ...
        name, " exceeds HIP int index range: ", value
    );
...
int64_t checked_sv_affine_alpha_size(int max_L1, int max_L2) {
    TORCH_CHECK(
        ...
        "sv_affine dimensions too large: L1 + L2 exceeds HIP int index range"
    );
    ...
    TORCH_CHECK(
        ...
        "sv_affine DP table too large: 3 * (L1 + 1) * (L2 + 1) exceeds HIP int index range"
    );
    return static_cast<int64_t>(alpha_size);
}

void validate_sv_affine_lengths_hip(
...
torch::Tensor resolve_sv_affine_lengths_hip(
    ...
    validate_sv_affine_lengths_hip(lengths, B, max_L1, max_L2, device);
    return lengths;
}
...
class SoftSVAffineHIPFunction : public torch::autograd::Function<SoftSVAffineHIPFunction> {
public:
    static torch::autograd::tensor_list forward(
        ...
        validate_sv_affine_lengths_hip(lengths, B, max_L1, max_L2, scores.device());
...
std::vector<torch::Tensor> soft_sv_affine_hip(
    torch::Tensor scores,
    torch::Tensor gap_open,
    torch::Tensor gap_ext,
    torch::Tensor temperature,
    ...
    ORIHIME_CHECK_INPUT_CUDA(scores);
    ORIHIME_CUDA_GUARD(scores);
    return SoftSVAffineHIPFunction::apply(scores, gap_open, gap_ext, temperature, lengths);
}

// Affine SW with float params (convenience function)
// Returns (score, alignment) - both differentiable
std::vector<torch::Tensor> soft_sv_affine_hip_float(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temperature,
    ...
    torch::Tensor lengths = resolve_sv_affine_lengths_hip(
        lengths_opt, B, L1, L2, scores.device()
    );

    return SoftSVAffineHIPFunction::apply(scores, gap_open_t, gap_ext_t, temp_t, lengths);
}

// Affine SW with explicit gradients
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
soft_sv_affine_hip_with_grads(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temperature,
...
// Affine SW HVP
torch::Tensor soft_sv_affine_hvp_hip(
    torch::Tensor scores,
...
torch::Tensor soft_sv_affine_param_jacobian_hip(
    torch::Tensor scores,
...
soft_sv_affine_backward_full_hip(
    torch::Tensor scores,
...
std::vector<torch::Tensor> orihime_sv_affine_forward_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sv_affine_hip_float(scores, gap_open, gap_ext, temp, lengths);
}

// sv_affine::forward_t - tensor params version
std::vector<torch::Tensor> orihime_sv_affine_forward_t_hip(
    torch::Tensor scores,
    torch::Tensor gap_open,
    torch::Tensor gap_ext,
    torch::Tensor temp,
    ...
    return soft_sv_affine_hip(scores, gap_open, gap_ext, temp, lengths);
}

// sv_affine::value_grad_params - returns (grad_gap_open, grad_gap_ext, grad_temp) per batch
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> orihime_sv_affine_value_grad_params_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    auto result = soft_sv_affine_hip_with_grads(scores, gap_open, gap_ext, temp, lengths);
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> orihime_sv_affine_marginals_backward_hip(
    torch::Tensor scores,
    ...
    return soft_sv_affine_backward_full_hip(scores, grad_marginals, gap_open, gap_ext, temp, lengths);
}

// sv_affine::marginals_hvp - Hessian-vector product
torch::Tensor orihime_sv_affine_marginals_hvp_hip(
    torch::Tensor scores,
    torch::Tensor v,
    double gap_open,
    double gap_ext,
    ...
    return soft_sv_affine_hvp_hip(scores, v, gap_open, gap_ext, temp, lengths);
}

// sv_affine::marginals_grad_gap_open - d(marginals)/d(gap_open)
torch::Tensor orihime_sv_affine_marginals_grad_gap_open_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sv_affine_param_jacobian_hip(scores, 0, gap_open, gap_ext, temp, lengths);
}

// sv_affine::marginals_grad_gap_ext - d(marginals)/d(gap_ext)
torch::Tensor orihime_sv_affine_marginals_grad_gap_ext_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sv_affine_param_jacobian_hip(scores, 1, gap_open, gap_ext, temp, lengths);
}

// sv_affine::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor orihime_sv_affine_marginals_grad_temp_hip(
    torch::Tensor scores,
    double gap_open,
    double gap_ext,
    double temp,
    ...
    return soft_sv_affine_param_jacobian_hip(scores, 2, gap_open, gap_ext, temp, lengths);
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    ...
    m.impl("soft_sv_affine", soft_sv_affine_hip);
    m.impl("soft_sv_affine_float", soft_sv_affine_hip_float);
    m.impl("soft_sv_affine_with_grads", soft_sv_affine_hip_with_grads);
    m.impl("soft_sv_affine_hvp", soft_sv_affine_hvp_hip);
    m.impl("soft_sv_affine_param_jacobian", soft_sv_affine_param_jacobian_hip);
    m.impl("soft_sv_affine_backward_full", soft_sv_affine_backward_full_hip);

    // sv_affine namespace
    m.impl("sv_affine_forward", orihime_sv_affine_forward_hip);
    m.impl("sv_affine_forward_t", orihime_sv_affine_forward_t_hip);
    m.impl("sv_affine_value_grad_params", orihime_sv_affine_value_grad_params_hip);
    m.impl("sv_affine_marginals_backward", orihime_sv_affine_marginals_backward_hip);
    m.impl("sv_affine_marginals_hvp", orihime_sv_affine_marginals_hvp_hip);
    m.impl("sv_affine_marginals_grad_gap_open", orihime_sv_affine_marginals_grad_gap_open_hip);
    m.impl("sv_affine_marginals_grad_gap_ext", orihime_sv_affine_marginals_grad_gap_ext_hip);
    m.impl("sv_affine_marginals_grad_temp", orihime_sv_affine_marginals_grad_temp_hip);
}
