...
/**
 * @file torch_hip.cpp
 * @brief Needleman-Wunsch HIP Extension with PyTorch Autograd
 *
 * Global alignment with linear gap penalty. HIP implementations are registered
...
int64_t nw_alpha_size_hip(int max_L1, int max_L2) {
...
torch::Tensor resolve_nw_lengths_hip(
    ...
    validate_nw_lengths_hip(lengths, B, max_L1, max_L2, device);
    return lengths;
}
...
public:
    static torch::autograd::tensor_list forward(
        ...
        int64_t alpha_size = nw_alpha_size_hip(max_L1, max_L2);

        validate_nw_lengths_hip(lengths, B, max_L1, max_L2, scores.device());
...
std::vector<torch::Tensor> soft_nw_hip(
    torch::Tensor scores,
    torch::Tensor gap,
    torch::Tensor temperature,
...
std::vector<torch::Tensor> soft_nw_hip_float(
    torch::Tensor scores,
    double gap,
    double temperature,
    ...
    torch::Tensor lengths = resolve_nw_lengths_hip(lengths_opt, B, L1, L2, scores.device());
...
soft_nw_hip_with_grads(
    ...
    auto options = scores.options();
    torch::Tensor lengths = resolve_nw_lengths_hip(
...
torch::Tensor soft_nw_param_jacobian_hip(
    torch::Tensor scores,
...
std::vector<torch::Tensor> nw_forward_impl_hip(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_nw_hip_float(scores, gap, temp, lengths);
}

// nw::forward_t - tensor params version
std::vector<torch::Tensor> nw_forward_t_hip(
    torch::Tensor scores,
    torch::Tensor gap,
    torch::Tensor temp,
    torch::Tensor lengths
) {
    return soft_nw_hip(scores, gap, temp, lengths);
}

// nw::value_grad_params - returns (grad_gap, grad_temp) per batch
std::tuple<torch::Tensor, torch::Tensor> nw_value_grad_params_hip(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    auto result = soft_nw_hip_with_grads(scores, gap, temp, lengths);
    return std::make_tuple(std::get<2>(result), std::get<3>(result));
}

// nw::marginals_backward - full backward through marginals
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor> nw_marginals_backward_hip(
    torch::Tensor scores,
    ...
    return soft_nw_backward_full_hip(scores, grad_marginals, gap, temp, lengths);
}

// nw::marginals_hvp - Hessian-vector product
torch::Tensor nw_marginals_hvp_hip(
    torch::Tensor scores,
    torch::Tensor v,
    double gap,
    double temp,
    ...
    return soft_nw_hvp_hip(scores, v, gap, temp, lengths);
}

// nw::marginals_grad_gap - d(marginals)/d(gap)
torch::Tensor nw_marginals_grad_gap_hip(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_nw_param_jacobian_hip(scores, 0, gap, temp, lengths);
}

// nw::marginals_grad_temp - d(marginals)/d(temperature)
torch::Tensor nw_marginals_grad_temp_hip(
    torch::Tensor scores,
    double gap,
    double temp,
    c10::optional<torch::Tensor> lengths
) {
    return soft_nw_param_jacobian_hip(scores, 1, gap, temp, lengths);
}
...
TORCH_LIBRARY_IMPL(orihime, CUDA, m) {
    m.impl("soft_nw", soft_nw_hip);
    m.impl("soft_nw_float", soft_nw_hip_float);
    m.impl("soft_nw_with_grads", soft_nw_hip_with_grads);
    m.impl("soft_nw_hvp", soft_nw_hvp_hip);
    m.impl("soft_nw_param_jacobian", soft_nw_param_jacobian_hip);
    m.impl("soft_nw_backward_full", soft_nw_backward_full_hip);

    // Namespaced API
    m.impl("nw_forward", nw_forward_impl_hip);
    m.impl("nw_forward_t", nw_forward_t_hip);
    m.impl("nw_value_grad_params", nw_value_grad_params_hip);
    m.impl("nw_marginals_backward", nw_marginals_backward_hip);
    m.impl("nw_marginals_hvp", nw_marginals_hvp_hip);
    m.impl("nw_marginals_grad_gap", nw_marginals_grad_gap_hip);
    m.impl("nw_marginals_grad_temp", nw_marginals_grad_temp_hip);
}
